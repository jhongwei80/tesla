...
import io.github.tesla.auth.server.oauth.service.OAuthService;

/**
 * 2015/7/3
 * <p/>
 * 对OAUTH各种流程的操作进行抽象, 将没用的行为(方法) 放于此
 */
public abstract class OAuthHandler {
    ...
    protected OAuthResponse createTokenResponse(AccessToken accessToken, boolean queryOrJson)
        ...
        return createTokenResponse(accessToken, queryOrJson, tempClientDetails);
    }

    public static OAuthResponse createTokenResponse(AccessToken accessToken, boolean queryOrJson,
        ClientDetails tempClientDetails) throws OAuthSystemException {

        final OAuthASResponse.OAuthTokenResponseBuilder builder =
            OAuthASResponse.tokenResponse(HttpServletResponse.SC_OK).location(tempClientDetails.getRedirectUri())
