...
public class OAuth2AccessTokenJsonExtractor implements TokenExtractor<OAuth2AccessToken> {
    ...
    private static final Pattern REFRESH_TOKEN_EXPIRES_IN_REGEX_PATTERN =
        Pattern.compile("\"refresh_token_expires_in\"\\s*:\\s*\"?(\\d*?)\"?\\D");
    ...
    private OAuth2AccessToken createToken(String response) {
        ...
        final String refreshTokenExpiresInString =
            extractParameter(response, REFRESH_TOKEN_EXPIRES_IN_REGEX_PATTERN, false);
        Integer refreshTokenExpiresIn;
        try {
            refreshTokenExpiresIn =
                refreshTokenExpiresInString == null ? null : Integer.valueOf(refreshTokenExpiresInString);
        } catch (NumberFormatException nfe) {
            refreshTokenExpiresIn = null;
        }

        final String scope = extractParameter(response, SCOPE_REGEX_PATTERN, false);
        return createToken(accessToken, tokenType, expiresIn, refreshToken, refreshTokenExpiresIn, scope, response);
    }

    protected OAuth2AccessToken createToken(String accessToken, String tokenType, Integer expiresIn,
        String refreshToken, Integer refreshTokenExpiresIn, String scope, String response) {
        return new OAuth2AccessToken(accessToken, tokenType, expiresIn, refreshToken, refreshTokenExpiresIn, scope,
            response);
    }
