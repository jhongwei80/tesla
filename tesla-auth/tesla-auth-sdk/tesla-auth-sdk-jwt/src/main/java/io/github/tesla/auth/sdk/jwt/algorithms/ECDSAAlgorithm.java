...
class ECDSAAlgorithm extends Algorithm {
    ...
    public void verify(DecodedJWT jwt) throws SignatureVerificationException {
        byte[] contentBytes =
            String.format("%s.%s", jwt.getHeader(), jwt.getPayload()).getBytes(StandardCharsets.UTF_8);
        ...
        try {
            ...
            boolean valid =
                crypto.verifySignatureFor(getDescription(), publicKey, contentBytes, JOSEToDER(signatureBytes));
    ...
    byte[] DERToJOSE(byte[] derSignature) throws SignatureException {
        ...
        // Retrieve R number
        System.arraycopy(derSignature, offset + Math.max(-rPadding, 0), joseSignature, Math.max(rPadding, 0),
