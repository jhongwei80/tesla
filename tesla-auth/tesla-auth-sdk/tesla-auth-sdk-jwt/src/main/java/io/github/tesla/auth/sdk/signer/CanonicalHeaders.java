    ...
    static class Builder {
        ...
        CanonicalHeaders build() {
            ...
            internalMap.entrySet()
                .forEach(header -> canonicalizedHeadersBuilder.append(header.getKey().toLowerCase()).append(':')
                    .append(
                        header.getValue().stream().map(Builder::normalizeHeaderValue).collect(HEADER_VALUE_COLLECTOR))
                    .append('\n'));
