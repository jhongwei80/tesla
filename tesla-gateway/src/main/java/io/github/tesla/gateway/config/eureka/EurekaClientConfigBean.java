...
/**
 * @author liushiming
 * @version EurekaClientConfigBean.java, v 0.0.1 2018年5月4日 下午6:15:36 liushiming
 */
...
public class EurekaClientConfigBean implements EurekaClientConfig {
    ...
    /**
     * Indicates how often(in seconds) to poll for changes to eureka server information. Eureka servers could be added
     * or removed and this setting controls how soon the eureka clients should know about it.
     */
    ...
    /**
     * Indicates how long to wait (in seconds) before a connection to eureka server needs to timeout. Note that the
     * connections in the client are pooled by org.apache.http.client.HttpClient and this setting affects the actual
     * connection creation and also the wait time to get the connection from the pool.
     */
    private int eurekaServerConnectTimeoutSeconds = 5;

    /**
     * Gets the name of the implementation which implements BackupRegistry to fetch the registry information as a fall
     * back option for only the first time when the eureka client starts.
     * <p>
     * This may be needed for applications which needs additional resiliency for registry information without which it
     * cannot operate.
     */
    ...
    /**
     * Gets the URL context to be used to construct the service url to contact eureka server when the list of eureka
     * servers come from the DNS. This information is not required if the contract returns the service urls from
     * eurekaServerServiceUrls.
     * <p>
     * The DNS mechanism is used when useDnsForFetchingServiceUrls is set to true and the eureka client expects the DNS
     * to configured a certain way so that it can fetch changing eureka servers dynamically. The changes are effective
     * at runtime.
     */
    private String eurekaServerURLContext;

    /**
     * Gets the port to be used to construct the service url to contact eureka server when the list of eureka servers
     * come from the DNS.This information is not required if the contract returns the service urls
     * eurekaServerServiceUrls(String).
     * <p>
     * The DNS mechanism is used when useDnsForFetchingServiceUrls is set to true and the eureka client expects the DNS
     * to configured a certain way so that it can fetch changing eureka servers dynamically.
     * <p>
    ...
    /**
     * Gets the DNS name to be queried to get the list of eureka servers.This information is not required if the
     * contract returns the service urls by implementing serviceUrls.
     * <p>
     * The DNS mechanism is used when useDnsForFetchingServiceUrls is set to true and the eureka client expects the DNS
     * to configured a certain way so that it can fetch changing eureka servers dynamically.
     * <p>
    ...
    /**
     * Indicates how much time (in seconds) that the HTTP connections to eureka server can stay idle before it can be
     * closed.
     * <p>
     * In the AWS environment, it is recommended that the values is 30 seconds or less, since the firewall cleans up the
     * connection information after a few mins leaving the connection hanging in limbo
     */
    ...
    /**
     * Heartbeat executor exponential back off related property. It is a maximum multiplier value for retry delay, in
     * case where a sequence of timeouts occurred.
     */
    ...
    /**
     * Cache refresh executor exponential back off related property. It is a maximum multiplier value for retry delay,
     * in case where a sequence of timeouts occurred.
     */
    private int cacheRefreshExecutorExponentialBackOffBound = 10;

    /**
     * Map of availability zone to list of fully qualified URLs to communicate with eureka server. Each value can be a
     * single URL or a comma separated list of alternative locations.
     * <p>
     * Typically the eureka server URLs carry protocol,host,port,context and version information if any. Example:
    ...
    /**
     * Indicates whether the content fetched from eureka server has to be compressed whenever it is supported by the
     * server. The registry information from the eureka server is compressed for optimum network traffic.
     */
     ...
     * When the DNS name is updated to have additional servers, that information is used immediately after the eureka
     * client polls for that information as specified in eurekaServiceUrlPollIntervalSeconds.
     * <p>
     * Alternatively, the service urls can be returned serviceUrls, but the users should implement their own mechanism
     * to return the updated list in case of changes.
     * <p>
     * The changes are effective at runtime.
     */
    ...
    /**
     * Indicates whether or not this instance should register its information with eureka server for discovery by
     * others.
     * <p>
    ...
    /**
     * Indicates whether or not this instance should try to use the eureka server in the same zone for latency and/or
     * other reason.
     * <p>
    ...
    /**
     * Indicates whether to log differences between the eureka server and the eureka client in terms of registry
     * information.
     * <p>
     * Eureka client tries to retrieve only delta changes from eureka server to minimize network traffic. After
     * receiving the deltas, eureka client reconciles the information from the server to verify it has not missed out
     * some information. Reconciliation failures could happen when the client has had network issues communicating to
     * server.If the reconciliation fails, eureka client gets the full registry information.
     * <p>
     * While getting the full registry information, the eureka client can log the differences between the client and the
     * server and this setting controls that.
     * <p>
     * The changes are effective at runtime at the next registry fetch cycle as specified by
    ...
    /**
     * Indicates whether the eureka client should disable fetching of delta and should rather resort to getting the full
     * registry information.
     * <p>
     * Note that the delta fetches can reduce the traffic tremendously, because the rate of change with the eureka
     * server is normally much lower than the rate of fetches.
     * <p>
     * The changes are effective at runtime at the next registry fetch cycle as specified by
     * registryFetchIntervalSeconds
     */
     ...
     * define the availability zones for each of these regions as returned by availabilityZones. Failing to do so, will
     * result in failure of discovery client startup.
     */
    ...
    /**
     * Indicates whether to get the applications after filtering the applications for instances with only InstanceStatus
     * UP states.
     */
    ...
    /**
     * Indicates whether server can redirect a client request to a backup server/cluster. If set to false, the server
     * will handle the request directly, If set to true, it may send HTTP redirect to the client, with a new server
     * location.
     */
    private boolean allowRedirects = false;

    /**
     * If set to true, local status updates via ApplicationInfoManager will trigger on-demand (but rate limited)
     * register/updates to remote eureka servers
     */
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(propertyResolver, that.propertyResolver) && enabled == that.enabled
            ...
            && allowRedirects == that.allowRedirects && onDemandUpdateStatusChange == that.onDemandUpdateStatusChange
    ...
    public int hashCode() {
        return Objects.hash(propertyResolver, enabled, transport, registryFetchIntervalSeconds,
            ...
            eurekaServerTotalConnections, eurekaServerTotalConnectionsPerHost, eurekaServerURLContext, eurekaServerPort,
            ...
            cacheRefreshExecutorThreadPoolSize, cacheRefreshExecutorExponentialBackOffBound, serviceUrl, gZipContent,
            useDnsForFetchingServiceUrls, registerWithEureka, preferSameZoneEureka, logDeltaDiff, disableDelta,
            fetchRemoteRegionsRegistry, availabilityZones, filterOnlyUpInstances, fetchRegistry, dollarReplacement,
            escapeCharReplacement, allowRedirects, onDemandUpdateStatusChange, encoderName, decoderName,
            clientDataAccept);
    }
    ...
    public String toString() {
        return new StringBuilder("EurekaClientConfigBean{").append("propertyResolver=").append(propertyResolver)
            ...
            .append("proxyPort='").append(proxyPort).append("', ").append("proxyHost='").append(proxyHost).append("', ")
            .append("proxyUserName='").append(proxyUserName).append("', ").append("proxyPassword='")
            ...
            .append("', ").append("eurekaServerDNSName='").append(eurekaServerDNSName).append("', ").append("region='")
            .append(region).append("', ").append("eurekaConnectionIdleTimeoutSeconds=")
            ...
            .append(heartbeatExecutorThreadPoolSize).append(", ").append("heartbeatExecutorExponentialBackOffBound=")
            .append(heartbeatExecutorExponentialBackOffBound).append(", ").append("cacheRefreshExecutorThreadPoolSize=")
            .append(cacheRefreshExecutorThreadPoolSize).append(", ")
            .append("cacheRefreshExecutorExponentialBackOffBound=").append(cacheRefreshExecutorExponentialBackOffBound)
            .append(", ").append("serviceUrl=").append(serviceUrl).append(", ").append("gZipContent=")
            .append(gZipContent).append(", ").append("useDnsForFetchingServiceUrls=")
            .append(useDnsForFetchingServiceUrls).append(", ").append("registerWithEureka=").append(registerWithEureka)
            .append(", ").append("preferSameZoneEureka=").append(preferSameZoneEureka).append(", ")
            .append("logDeltaDiff=").append(logDeltaDiff).append(", ").append("disableDelta=").append(disableDelta)
            .append(", ").append("fetchRemoteRegionsRegistry='").append(fetchRemoteRegionsRegistry).append("', ")
            .append("availabilityZones=").append(availabilityZones).append(", ").append("filterOnlyUpInstances=")
            .append(filterOnlyUpInstances).append(", ").append("fetchRegistry=").append(fetchRegistry).append(", ")
            .append("dollarReplacement='").append(dollarReplacement).append("', ").append("escapeCharReplacement='")
            .append(escapeCharReplacement).append("', ").append("allowRedirects=").append(allowRedirects).append(", ")
            ...
            .append("', ").append("clientDataAccept='").append(clientDataAccept).append("'").append("}").toString();
    }
}
